//
// Created by Benedikt Kluss on 06.01.21.
//
#include <stdio.h>
#include "../../lib/TestLibrary.h"

#include "../src/keyExpansion/keyExpansion.h"

int test_expansion128BitKey () {
    int result = 0;

    unsigned char key[] = {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c};
    unsigned char expectedExpandedKey[] = {
            
    };

    unsigned char * expandedKey = getExpanded128bitKey(key);

    for (int i = 0; i < 176; ++i) {
        result += assertExpectedEqualsActual(expectedExpandedKey[i], expandedKey[i]);
        //printf("%d: e: %x a: %x \n", i, expectedExpandedKey[i], expandedKey[i]);
    }

    return result;
}

int main() {
    int result = 0;

    result += test_expansion128BitKey();

    return  result;
}